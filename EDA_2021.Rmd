...
data %>% select(winner_id) %>% group_by(winner_id) %>% count(sort = TRUE) %>% View()

players %>% filter(player_id == 106421)
...
#Missing data for winner seed and loser seed is appropriate because not everyone is seeded
#We will replace these values with 0 for unseeded
...
#Replace the blanks with NAs
data <- data %>% 
  replace_with_na_at(.vars = c("winner_entry", "loser_entry"),
                     condition = ~.x == "")

#For some reason, heights are missing for a lot of people...let's inspect
...
```{r}
#In python I'd create a dictionary, we will use a pseudo-equivalent here
#Had to manually look up heights which was really annoying and still had some NA's. Tried to pull heights from ATP website whenever possible.
...
data3$score1 <- data3$score
data3 <- separate(data3, score1, into = c("set_1", "set_2", "set_3", "set_4", "set_5"), sep = " ")
...
##We want to make a feature saying if the winner or the loser won the tiebreaks of each set
...
##Let's do a test exploration

```{r}
#If the first set goes to a tiebreak, how often does the winner of the first set tiebreak win the match?
#Dependent variable - winning match
#Independent variable - winning first set TB
...
```{r}
#If the first set goes to a tiebreak, how often does the winner get a second set get a bagel or breadstick?
#Did tiebreaker winner have the bakery 
...
##Additional more basic EDA

```{r}

#Distribution of heights in ATP 2021
