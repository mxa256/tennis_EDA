...
mean_height <- mean(heights_df$Height, na.rm = T)
...
data4$w_point_dom <- (data4$w_1stWon+data4$w_2ndWon) / data4$w_svpt
data4$l_point_dom <- (data4$l_1stWon+data4$l_2ndWon) / data4$l_svpt
...
data4$w_bp_saved_op_ratio <- data4$w_bp_saved_perc / data4$w_1st_serve_perc_win

data4$l_bp_saved_perc <- data4$l_bpSaved / data4$l_bpFaced
data4$l_bp_saved_op_ratio <- data4$l_bp_saved_perc / data4$l_1st_serve_perc_win

#Break point converted over performing ratio
data4$w_bp_convert_op_ratio <- data4$w_bp_convert_perc / data4$w_return_perc_win
data4$l_bp_convert_op_ratio <- data4$l_bp_convert_perc / data4$l_return_perc_win
...
data7_tophalf <- data7[1:rows_split,]
data7_bottomhalf <- data7[(rows_split+1):nrow(data7),]

data7_tophalf$player <- ifelse(data7_tophalf$p1_won, "P_1", "P_2")
data7_bottomhalf$player <- ifelse(data7_bottomhalf$p1_won, "P_2", "P_1")
...
data8 <- data7 %>% pivot_wider(id_cols = c("tourney_id", "match_num"), 
                               names_from = "player", 
                               values_from = c("id", "name", "Win", "tourney_date", "tourney_name", "tourney_level", "draw_size", "surface", "score", "best_of", "round", "year", "set_1", "TB_1", "set_2", "TB_2", "set_3", "TB_3", "set_4", "TB_4", "set_5", "TB_5", "rank_diff", "seed", "hand", "ht", "age", rolled_up))
...
#We'll work with the compact data in longer format as well -- we won't change the name
...
data7 <- data7 %>% select(!remove_data7)
...
ggplot(data7, aes(x = point_dom_av, y = gmstosets_op_ratio_av, color = Win)) +
  geom_jitter() +
  geom_density2d() +
  #facet_grid(. ~ Win) +
...
data12 <- data7 %>% select(Win, point_dom_av, gmstosets_op_ratio_av, set_tot_av, pts2sets_op_ratio_av, gmstosets_op_ratio_av)
